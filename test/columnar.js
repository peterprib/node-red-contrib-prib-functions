...
describe('Columnar Store', function() {
    it('writes, reads and queries using columnar rid maps or filters', async function() {
        ...
        // Basic SQL query
        const sqlResult = await SimpleColumnarStore.sqlQuery(tmp, "SELECT * FROM ? WHERE age > 25");
        sqlResult.length.should.equal(3);

        // SQL with column projection
        const projected = await SimpleColumnarStore.sqlQuery(tmp, "SELECT id, name FROM ? WHERE age > 30");
        ...
        // SQL with ORDER BY
        const ordered = await SimpleColumnarStore.sqlQuery(tmp, "SELECT * FROM ? ORDER BY age DESC");
        ordered.length.should.equal(4);
        ordered[0].age.should.equal(40);
        ordered[3].age.should.equal(20);

        // SQL with LIMIT
        const limited = await SimpleColumnarStore.sqlQuery(tmp, "SELECT * FROM ? ORDER BY age DESC LIMIT 2");
        limited.length.should.equal(2);

        // SQL with GROUP BY and COUNT
        const grouped = await SimpleColumnarStore.sqlQuery(tmp, "SELECT dept, COUNT(*) AS cnt FROM ? GROUP BY dept");
        grouped.length.should.equal(2);
        const engGroup = grouped.find(g => g.dept === 'eng');
        engGroup.should.have.property('cnt', 2);

        // SQL with aggregates
        const agg = await SimpleColumnarStore.sqlQuery(tmp, "SELECT AVG(age) AS avg_age, MAX(age) AS max_age, MIN(age) AS min_age FROM ?");
        ...
        const sumAgg = await SimpleColumnarStore.sqlQuery(tmp, "SELECT dept, SUM(age) AS total_age FROM ? GROUP BY dept");
    ...
    it('supports INNER, LEFT, RIGHT, and FULL OUTER JOINs', async function() {
        ...
        [file1, file2].forEach(f => { if (fs.existsSync(f)) fs.unlinkSync(f); });
        ...
        const innerJoin = await SimpleColumnarStore.sqlQuery(file1, 
        ...
        const leftJoin = await SimpleColumnarStore.sqlQuery(file1,
        ...
        const rightJoin = await SimpleColumnarStore.sqlQuery(file1,
        ...
        const fullJoin = await SimpleColumnarStore.sqlQuery(file1,
    ...
    it('supports appending records to existing files', async function() {
        ...
        const writeResult = await SimpleColumnarStore.writeRecords(initialRecords, tmp);
        ...
        const appendResult = await SimpleColumnarStore.appendRecords(appendRecords, tmp);
        ...
        const newAppendResult = await SimpleColumnarStore.appendRecords([{id: 5, name: 'Eve'}], newFile);
        ...
        // Clean up
        [tmp, newFile].forEach(f => { if (fs.existsSync(f)) fs.unlinkSync(f); });
    });
    ...
    it('supports hardcoded SQL queries in node configuration', async function() {
        ...
        const result2 = await SimpleColumnarStore.sqlQuery(filePath, mockMsg.payload.sql);
    ...
    it('demonstrates SQL parsing performance improvement with caching', async function() {
        ...
        const sql = "SELECT dept, COUNT(*) as cnt, AVG(salary) as avg_salary FROM ? GROUP BY dept ORDER BY cnt DESC";
    ...
    it('supports configurable output property for results', async function() {
        ...
        const sqlResult = await SimpleColumnarStore.sqlQuery(tmp, "SELECT name FROM ? WHERE age > 26");
